...
function BonusLeaderboard({ entries, loading, onRefresh }: { entries: BonusLeaderboardEntry[]; loading: boolean; onRefresh: () => Promise<void> }) {
  ...
  useEffect(() => {
    if (autoSelected.current || loading || !entries.length) return;
    const preferred = RECORD_GAME_LABELS.find((game) => entries.some((entry) => entry.gameId === game.id && entry.isCurrent))
      || RECORD_GAME_LABELS.find((game) => entries.some((entry) => entry.gameId === game.id));
    if (preferred) setGameId(preferred.id);
    autoSelected.current = true;
  }, [entries, loading]);
  return <section className="bonus-leaderboard"><div className="leaderboard-heading"><div><small>HISTORIAL COLABORATIVO</small><h3>Salón de récords</h3><p>{entries.length ? `${entries.length} resultado${entries.length === 1 ? "" : "s"} destacado${entries.length === 1 ? "" : "s"} entre todos los participantes.` : "Los mejores resultados de todos los participantes."}</p></div><button type="button" disabled={loading} onClick={onRefresh}>{loading ? "Actualizando…" : "↻ Actualizar"}</button></div><div className="leaderboard-tabs">{RECORD_GAME_LABELS.map((game) => { const count = entries.filter((entry) => entry.gameId === game.id).length; return <button type="button" className={game.id === gameId ? "active" : ""} key={game.id} onClick={() => setGameId(game.id)}>{game.label}<span>{count}</span></button>; })}</div>{loading && !ranking.length ? <div className="leaderboard-empty loading">Preparando el ranking…</div> : ranking.length ? <div className="leaderboard-list">{ranking.map((entry, index) => <article className={entry.isCurrent ? "current" : ""} key={`${entry.gameId}-${entry.name}-${entry.completedAt}`}><span className={`rank-position rank-${index + 1}`}>{index + 1}</span><div><b>{entry.name}{entry.isCurrent ? " · Tú" : ""}</b><small>{entry.uad || "Colaborador"}</small></div><strong>{entry.record}<small>{current.unit}</small></strong>{entry.isCurrent && <em>TU MEJOR</em>}</article>)}</div> : <div className="leaderboard-empty"><i>☆</i><b>Aún no hay récords en {current.label}</b><span>{entries.length ? "Selecciona una categoría que tenga resultados." : "¡Sé la primera persona en aparecer aquí!"}</span></div>}</section>;
}
...
function StationPairIllustration({ station }: { station: string }) {
  if (station === "diversidad") return <svg viewBox="0 0 100 76" aria-hidden="true"><circle cx="27" cy="27" r="10" /><circle cx="50" cy="21" r="11" /><circle cx="73" cy="27" r="10" /><path d="M11 63c2-16 10-23 18-23s15 7 17 23M38 62c1-19 6-29 12-29s12 10 13 29M55 63c2-16 9-23 17-23s16 7 18 23" /></svg>;
  if (station === "felicidad") return <svg viewBox="0 0 100 76" aria-hidden="true"><circle cx="50" cy="38" r="18" /><path d="M50 7v10M50 59v10M19 38h11M70 38h11M28 16l8 8M64 52l8 8M72 16l-8 8M36 52l-8 8" /><path d="M42 37h.1M58 37h.1M42 45c5 5 11 5 16 0" /></svg>;
  if (station === "seguridad") return <svg viewBox="0 0 100 76" aria-hidden="true"><path d="M50 8 78 19v18c0 17-12 27-28 33-16-6-28-16-28-33V19L50 8Z" /><path d="m35 39 10 10 21-23" /></svg>;
  if (station === "salud") return <svg viewBox="0 0 100 76" aria-hidden="true"><path d="M50 66S17 48 17 27c0-18 23-23 33-7 10-16 33-11 33 7 0 21-33 39-33 39Z" /><path d="M26 39h14l6-13 9 27 7-14h13" /></svg>;
  if (station === "amor") return <svg viewBox="0 0 100 76" aria-hidden="true"><path d="M50 64S24 49 24 31c0-15 19-19 26-6 7-13 26-9 26 6 0 18-26 33-26 33Z" /><path d="m19 16 3 6 6 3-6 3-3 6-3-6-6-3 6-3 3-6ZM82 43l2 4 4 2-4 2-2 4-2-4-4-2 4-2 2-4Z" /></svg>;
  return <svg viewBox="0 0 100 76" aria-hidden="true"><path d="M50 66V37M50 42c-18-1-27-9-28-25 17-1 27 7 28 25ZM50 50c19-1 29-10 30-27-18-1-29 8-30 27Z" /><path d="M37 67h27M32 60c9-5 27-5 36 0" /></svg>;
}
...
function StationPairsGame({ bestRecord, busy, onComplete }: { bestRecord: number; busy: boolean; onComplete: (score: number, record: number) => Promise<void> }) {
  ...
  return <div className="game-body pairs-game"><div className="arcade-hud"><span><small>TIEMPO</small><b>{seconds}s</b></span><span><small>MOVIMIENTOS</small><b>{moves}</b></span><span><small>TU RÉCORD</small><b>{bestRecord} pts</b></span></div>{phase === "ready" ? <div className="pairs-intro"><span>✦</span><h4>Encuentra las seis estaciones</h4><p>Cada estación tiene una ilustración propia. Memoriza las imágenes y encuentra sus dos cartas iguales.</p><button className="game-complete-button" onClick={start}>Comenzar juego</button></div> : <div className="pairs-board">{deck.map((card, index) => { const visible = open.includes(index) || matched.includes(card.id); const isMatched = matched.includes(card.id); return <button type="button" aria-label={visible ? card.name : "Carta oculta"} className={`${visible ? "revealed" : ""} ${isMatched ? "matched" : ""}`} style={{ "--pair-color": card.color } as React.CSSProperties} key={card.key} onClick={() => flip(index)}>{visible ? <span className="card-face" key={`face-${card.key}`}><span className="pair-illustration"><StationPairIllustration station={card.id} /></span><b>{card.name}</b>{isMatched && <em>✓</em>}</span> : <span className="card-back" key={`back-${card.key}`}><i /><b>?</b><small>PASAPORTE</small></span>}</button>; })}</div>}{phase === "finished" && <div className="pairs-result"><b>{record > bestRecord ? "¡Nuevo récord de memoria!" : "¡Todas las estaciones conectadas!"}</b><p>{moves} movimientos · {seconds} segundos · {record} puntos de desempeño.</p><div><button className="game-check-button" onClick={start}>Jugar otra vez</button><button className="game-complete-button" disabled={busy} onClick={() => onComplete(reward, record)}>{busy ? "Guardando…" : `Guardar récord · +${reward}`}</button></div></div>}</div>;
}
